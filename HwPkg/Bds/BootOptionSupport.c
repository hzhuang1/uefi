  ...
  )
{
    ...
    if(BDS_DEVICE_MAX <= DeviceType)
    {
        Print(L"Bds DeviceType UNSUPPORTED!!\n");
        Status=EFI_UNSUPPORTED;
    }
    
    Status=BdsLoadOptionSupportList[DeviceType].ListDevices (SupportedDeviceList);
    if (EFI_ERROR(Status)) {
        Print(L"Bds Load Option Support List fail\n");
        return Status;
      }
